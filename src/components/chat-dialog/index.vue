  ...
  <div id="mobile">
        ...
        >
          <div
            ...
            :style="
              msg.width &&
              msg.height && {
                width: msg.width - 26 + 'px',
                height: msg.height - 18 + 'px',
              }
            "
            :class="{
              ...
              animate_breathe:
                index === messages.length - 1 && status === 'componentClose',
            }"
    ...
    <div id="mobile-foot">
      ...
      >
        ...
        <var-col :span="4" style="padding: 0">
          <var-button
            ref="sendMsgBtnRef"
            type="success"
            size="small"
...
export default {
  ...
  methods: {
    buildMsgChain(messages) {
      messages.forEach(({ msgs, msgInputSpeed, author, triggerNextAction }) => {
        this.msgChain = this.msgChain.then(() =>
          this.sendSysMsg(msgs, msgInputSpeed, author, triggerNextAction)
        );
      });
    },
    ...
    sendUserMsg() {
      ...
      if (type === TRIGGER_NEXT_ACTION_TYPE.USER_INPUT) {
        if (this.rejectNextMsg(message, resolveKeyTexts, rejectKeyTexts)) {
          ...
          const rejectText =
            rejectHitTexts[Math.min(tryCnt, rejectHitTexts.length - 1)];
          let rejectSysMsgChain = Promise.resolve();
          if (Array.isArray(rejectText)) {
            // 多条回复
            rejectText.forEach((text) => {
              rejectSysMsgChain = rejectSysMsgChain.then(() =>
                this.sendSysMsg(text, inputSpeed)
              );
            });
          } else {
            // 单条回复
    ...
    rejectNextMsg(message, resolveKeyTexts = [], rejectKeyTexts = []) {
      if (
        rejectKeyTexts.some((rejectText) => message.indexOf(rejectText) > -1)
      ) {
        return true;
      }
      if (
        resolveKeyTexts.some((resolveKey) => message.indexOf(resolveKey) > -1)
      ) {
        return false;
      }
      return true;
    ...
    getProps(message, type) {
      ...
      if (messageDoms.length === 1) {
        ...
        attrs.forEach(
          (attrName) => (props[attrName] = messageDom.getAttribute(attrName))
        );
      }
      return props;
    },
...
function updateScroll() {
  ...
  const distance =
    $chatbox[0].scrollHeight - $chatbox.height() - $chatbox.scrollTop();
  const duration = 250;
